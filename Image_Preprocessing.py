...
def merge_images(mydate):
    dem_fps_mosaic = [
        join(mydate, f)
        for f in listdir(mydate)
        if isfile(join(mydate, f)) and "nfs00000004e55bbd9f00000001" not in str(f)
    ]
    src_files_mosaic_final = []
    for item in dem_fps_mosaic:
    ...
    outpath_root = (
        "/datacommons/carlsonlab/ld243/images/"
        + mydate.split("/")[-1]
        + "_mosaic_final.tif"
    )  #####
    with rasterio.open(
...
def main():
    sensor_loc = pd.read_pickle(r"/datacommons/carlsonlab/ld243/geometry_info/sensor_locations.pkl")
    mydir = sorted(os.listdir("/datacommons/carlsonlab/ld243/images/"))

    for path in tqdm(mydir, position=0, leave=True):
        print("Processing images: " + path)
        mypath = "/datacommons/carlsonlab/ld243/images/" + path
        if (
            len(listdir(mypath)) <= 38
        ):  # Filter the days with num_imgs <= 38 to save time and memory
            continue
        merged_image_path = merge_images(mypath)
        exact_shape_dir = (
            "/datacommons/carlsonlab/ld243/geometry_info/Delhi.geojson"
        )
        ...
        grid_transform(
            cropped_image_path, grid_size, path, save_path, sensor_loc=sensor_loc
        )
